...
sequelize
  ...
  .then(async () => {
    ...
    try {
      ...
      // Make models globally available for routes
      ...
      global.seedDatabase = seedDatabase;
...
app.post('/api/health', (req, res) => {
  ...
  res.json({ message: 'POST API Health route works!', status: 'healthy', data: req.body });
});
...
app.post('/api/cart', async (req, res) => {
  try {
    ...
    console.log('Request body:', JSON.stringify(req.body, null, 2));
    console.log('Adding to cart for user:', userId, 'pack:', packId, 'quantity:', quantity);

    // Validate required fields
    if (!userId || !packId || !quantity) {
      console.error('Missing required fields:', { userId, packId, quantity });
      return res.status(400).json({ error: 'userId, packId, and quantity are required' });
    }
    ...
    console.log('Existing cart item check:', existingCartItem ? 'Found' : 'Not found');
...
app.delete('/api/packs/:id/products', async (req, res) => {
   try {
     const result = await PackProduct.destroy({ where: { packId: req.params.id } });
     res.json({ message: 'Pack products deleted successfully', deletedCount: result });
   } catch (error) {
     res.status(500).json({ error: error.message });
   }
 });
...
app.listen(PORT, '0.0.0.0', async () => {
  ...
  console.log(`🔗 API Base URL: http://192.168.0.5:${PORT}/api`);
  console.log(`🌐 LAN Access: http://192.168.0.5:${PORT}/api`);
  ...
  if (modelsLoaded) {
    // Seed database on startup (optional - can be controlled with environment variable)
    if (process.env.SEED_ON_STARTUP === 'true') {
      ...
      console.log('ℹ️  Database seeding skipped (set SEED_ON_STARTUP=true to enable)');
    }
  } else {
